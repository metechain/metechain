...
func setAccount(bc *Blockchain, tx *transaction.FinishedTransaction, defaultAmount ...uint64) error {
	...
	// if availableBalance.Cmp(new(big.Int).Add(tx.Amount, tx.GasUsed)) == -1 {
	// 	return fmt.Errorf("availableBalance insufficient, availableBalance(%s)<amount(%s)+gas(%s) ", availableBalance, tx.Transaction.Amount, tx.GasUsed)
	// }
...
func (bc *Blockchain) setToAccount(block *block.Block, tx *transaction.Transaction) error {
	...
	// err = bc.handleMinedPledge(block, tx.To, tx.Amount)
	// if err != nil {
	// 	return err
	// }
...
func factCommit(sdb *state.StateDB, deleteEmptyObjects bool) (common.Hash, error) {
	/* 	sdb.Copy().Commit(deleteEmptyObjects) */
