...
import (
	...
	kevm "metachain/pkg/contract/evm"
...
func (s *Server) ServerHandler(ctx *fasthttp.RequestCtx) {
	...
	case "pledge":
		...
		totalP, err := s.getTotalPledge(a)
		if err != nil {
			errorCode = ErrData
			errMessage = err.Error()
			return
		}
		totalM, err := s.getTotalMined(a)
		if err != nil {
			errorCode = ErrData
			errMessage = err.Error()
			return
		}
		wholenet, err := s.GetWholeNet()
		if err != nil {
			errorCode = ErrData
			errMessage = err.Error()
			return
		}
		resBody, err = json.Marshal(&resultPledgeInfo{ErrorCode: 0, ErrorMsg: "ok", TotalPledge: totalP, TotalMined: totalM, WholeNetPledge: wholenet})
		if err != nil {
			errorCode = -1
			errMessage = err.Error()
			return
...
//get total pledge,usage {"method":"pledge","address":"otKDjCTT7fyrmraBwSV3pFkfY99rPziBWFBSSokHgjUQpGx"}
func (s *Server) getTotalPledge(address address.Address) (uint64, error) {
	return s.bc.GetTotalPledge(address)
}

//get total mined pledge,usage {"method":"pledge","address":"otKDjCTT7fyrmraBwSV3pFkfY99rPziBWFBSSokHgjUQpGx"}
func (s *Server) getTotalMined(address address.Address) (uint64, error) {
	return s.bc.GetTotalMined(address)
}

//get total mined pledge,usage {"method":"pledge","address":"otKDjCTT7fyrmraBwSV3pFkfY99rPziBWFBSSokHgjUQpGx"}
func (s *Server) GetWholeNet() (uint64, error) {
	return s.bc.GetWholeNetWorkTotalPledge()
}
