...
type IBlockchain interface {
	...
	//GetAllFreezeBalance(address address.Address) (*big.Int, error)
	//GetTotalPledge(address address.Address) (*big.Int, error)
	GetBindingMetaAddress(ethAddr string) (*common.Address, error)
	//GetSingleFreezeBalance(address.Address, address.Address) (*big.Int, error)
}
...
func (p *Pool) Pending() ([]transaction.SignedTransaction, error) {
	...
	// for ; i < PendingLimit && i < l; i++ {
	// 	st := p.q.pop()

	// 	if n := p.compareNonce(traderBuffer, st.Caller(), st.GetNonce()); n < 0 {
	// 		i--
	// 		l--
	// 		//p.getTraderInfo(traderBuffer, st.Caller())
...
func (p *Pool) transactionPreCalculated(traderBuf map[string]traderInfo, tx transaction.Transaction) error {
	...
	// case transaction.PledgeTrasnaction, transaction.EvmContractTransaction, transaction.EvmMetaTransaction:
...
// 	if caller.availableBalance.Cmp(tx.Amount) < 0 {
// 		return fmt.Errorf(err.Error()+": avaliable balance(%d) - lock amount(%d)", caller.availableBalance, tx.Amount)
// 	}
// 	avlBal := new(big.Int).Sub(caller.availableBalance, tx.Amount)
...
// 	// }
// 	if avlBal.Cmp(tx.GasFeeCap) == -1 {
// 		return fmt.Errorf(err.Error()+": avaliable balance(%s) - lock amount(%s) - gas(%s)", caller.availableBalance, tx.Amount, tx.GasCap())
// 	}
// 	avlBal = avlBal.Sub(avlBal, tx.GasCap())

// 	caller.nextNonce++
// 	caller.lockBalance = new(big.Int).Add(caller.lockBalance, tx.Amount)
...
// 	if caller.sigleLockBalance.Cmp(tx.Amount) == -1 {
// 		return fmt.Errorf("single locked balance(%s) < lock amount(%s)", caller.lockBalance, tx.Amount)
// 	}

// 	// lockBal, err := math.SubUint64Overflow(caller.lockBalance, tx.Amount)
// 	// if err != nil {
// 	if caller.lockBalance.Cmp(tx.Amount) == -1 {
// 		return fmt.Errorf(err.Error()+": locked balance(%s) - lock amount(%s)", caller.lockBalance, tx.Amount)
// 	}
// 	lockBal := new(big.Int).Sub(caller.lockBalance, tx.Amount)

// 	// singleBal, err := math.SubUint64Overflow(caller.sigleLockBalance, tx.Amount)
// 	// if err != nil {
// 	if caller.sigleLockBalance.Cmp(tx.Amount) == -1 {
// 		return fmt.Errorf(err.Error()+": single locked balance(%s) - lock amount(%s)", caller.lockBalance, tx.Amount)
// 	}
// 	singleBal := new(big.Int).Sub(caller.sigleLockBalance)
...
// 	pledge, err := p.bc.GetTotalPledge(tx.Caller())
// 	if err != nil {
// 		return err
// 	}

// 	if pledge <= 0 {
// 		return fmt.Errorf("no pledge balance:%d", pledge)
// 	}
