...
type Miner struct {
	...
	wg              sync.WaitGroup
	CoinbaseAddr    *common.Address   //矿工地址
	c               chan block.Block  //难度通道  接收block
	p               chan *block.Block //p2p通道  接收block
	r               chan *block.Block //rpc通道  接收block
	startch         chan bool
	download        chan bool
	started         bool
...
// func (m *Miner) calcNextMinerRequiredDifficulty(globalBits uint32, CoinbaseAddr address.Address) {
// 	globalTarget := CompactToBig(globalBits)

// 	//tenthTaget := big.NewInt(0).Div(globalTarget, big.NewInt(10))
// 	hashrateTarget := big.NewInt(0).Mul(globalTarget, big.NewInt(4))
// 	pledgeCoinbase := big.NewInt(0).Mul(globalTarget, big.NewInt(6))
// 	pledgeAmount, err := m.bc.GetTotalPledge(m.CoinbaseAddr)
// 	if err != nil {
// 		panic(err)
// 	}

// 	pleageTarget := pledgeCoinbase.Mul(pledgeCoinbase, big.NewInt(int64(maxPledge-pledgeAmount))).
// 		Div(pledgeCoinbase, big.NewInt(int64(maxPledge)))

// 	minerTarget := hashrateTarget.Add(hashrateTarget, pleageTarget).Div(hashrateTarget, big.NewInt(10))

// 	tmpBits := BigToCompact(minerTarget)

// 	atomic.CompareAndSwapUint32(&localBits, localBits, tmpBits)
// }
...
func (m *Miner) UpdateDifficultyFromLastBlock() (*block.Block, error) {
	...
	// pledge, err := m.bc.GetTotalPledge(m.CoinbaseAddr)
	// if err != nil {
	// 	logger.Error("get GetTotalPledge", zap.Error(err))
	// 	return nil, err
	// }
	...
	// newlocalBits, err := calcNextLocalRequiredDifficultyCopy(newGlobalBits, basePledge, pledge)
	// newlocalBits, err := difficulty.NextMinerDifficulty(newGlobalBits, pledge, basePledge)
	// if err != nil {
...
func (m *Miner) calcNextRequiredDifficulty(CoinbaseAddr *common.Address) error {
	...
	if tip != nil && tip.Height > 1 && (tip.Height-1)%Cycle == 0 {
		...
		// newGlobalBits := calcNextGlobalRequiredDifficultyCopy(int64(ob.Timestamp), int64(tip.Timestamp), globalBits)

		// newGlobalBits := calcNextGlobalRequiredDifficultyCopy(0, int64(subTime), globalBits)
		...
		// pledge, err := m.bc.GetTotalPledge(CoinbaseAddr)
		// if err != nil {
		// 	return err
		// }
		...
		//newlocalBits, err := difficulty.NextMinerDifficulty(CompactToBig(newGlobalBits), pledge, basePledge)
		// if err != nil {
		// 	return err
		// }
